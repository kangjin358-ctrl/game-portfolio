...
'<div class="nx-confirm">'+img("/nexus-cards/ui-confirm.webp","AR placement confirmation")+'<div><span>SCAN → POSITION → CONFIRM</span><h4>'+t("The card is both an input device and a spatial anchor.","卡牌既是输入设备，也是空间锚点。")+'</h4><p>'+t("Before confirmation the preview follows the tracked card. After confirmation the unit is re-parented to the battlefield while preserving world scale.","确认前预览跟随追踪卡；确认后单位重新挂载到战场并保持世界缩放。")+'</p></div></div>'+
title("03",c.levels,c.levelNote)+
'<div class="nx-levels">'+levels.map(l=>'<article><div class="nx-level-no">LEVEL '+l[0]+'</div><div class="nx-level-pair">'+img("/nexus-cards/level-"+l[0]+"-card.webp","Level "+l[0]+" card")+img("/nexus-cards/level-"+l[0]+"-ar.webp","Level "+l[0]+" AR")+'</div><h4>'+l[1]+'</h4><p>'+t(l[2],l[3])+'</p></article>').join("")+'</div>'+
title("04",c.systems)+
